...
public class JuryService {
    ...
    public Map<String, Object> create(CreateJuryRequest request) {
        ...
        List<JuryMember> members = request.members().stream()
                .map(m -> {
                    ...
                    jm.setId(UUID.randomUUID().toString());
                    jm.setJury(jury);
    ...
    public Map<String, Object> update(String id, Map<String, Object> updates) {
        ...
        if (updates.containsKey("templateId")) {
            JuryRoleTemplate template = juryRoleTemplateRepository.findById((String) updates.get("templateId"))
                    .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
                            "Template de rôle jury introuvable"));
            jury.setTemplate(template);
        }
        if (updates.containsKey("members")) {
            ...
            for (Map<String, String> m : memberData) {
                Teacher teacher = teacherRepository.findById(m.get("teacherId"))
                        .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST,
    ...
    private Map<String, Object> toResponse(Jury jury) {
        ...
        List<Map<String, String>> members = jury.getMembers().stream()
                .map(m -> {
